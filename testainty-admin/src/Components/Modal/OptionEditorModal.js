...
function OptionEditorModal({ isOpen, setIsOpen, optionIndex, setTextOptions, textOptions }) {
  ...
  //     updatedOptions[optionIndex][String.fromCharCode(65 + optionIndex)] = content;
  ...
  const handleChange = (content) => {
    console.log(content)
    const updatedOptions = [...textOptions].map((option, index) => {
      if (index === optionIndex) {
        return {
          ...option,
          [String.fromCharCode(65 + optionIndex)]: content,
  ...
  const handleImageUpload = async (blobInfo, success, failure) => {
    ...
    const api = `${process.env.REACT_APP_API_URL + apiConstants.UPLOAD_IMAGE_EDITOR}`
    try {
        ...
        <Editor
          ...
          value={
            (optionIndex === 0 || optionIndex 
                        && optionIndex < 4 && 
                        textOptions[optionIndex][String.fromCharCode(65 + optionIndex)]) ? 
              textOptions[optionIndex][String.fromCharCode(65 + optionIndex)] : 
              ''
          }
